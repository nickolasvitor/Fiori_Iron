...
sap.ui.define([
    ...
    function fnShowServiceError(oParams, oErrorTexts, oDisplayState) {
        if (!oDisplayState.bMessageOpen) {
            ...
            MessageBox.error(
                oErrorTexts.sErrorPortalUnavailable, {
    ...
    function fnShowSessionExpiredError(oParams, oErrorTexts, oDisplayState, oRouter) {
        if (!oDisplayState.bMessageOpen) {
            oDisplayState.bMessageOpen = true;
            oRouter.navTo("Login", true);
            MessageBox.error(
                oErrorTexts.sErrorSessionExpired, {
                title: oErrorTexts.sErrorTitle,                
                actions: [MessageBox.Action.CLOSE],
                onClose: function () {
                    oDisplayState.bMessageOpen = false;
        ...
        register: function (oComponent) {
            ...
            var oResourceBundle = oComponent.getModel("i18n").getResourceBundle(),
                oErrorTexts = {
                    ...
                    sErrorPortalUnavailable: oResourceBundle.getText("errorPortalUnavailable"),
                    sErrorSessionExpired: oResourceBundle.getText("errorSessionExpired")
                },
                oDisplayState = {
                    bMessageOpen: false
                };
            ...
            for (var sModel in oModels) {
                if (oModels[sModel].getMetadata().getName() === "sap.ui.model.odata.v2.ODataModel") {
                    ...
                    oModel.attachEvent("metadataFailed", function (oEvent) {
                        ...
                        fnShowServiceError(oParams, oErrorTexts, oDisplayState, oModel);
                    });
                    oModel.attachEvent("requestFailed", function (oEvent) {
                        var oParams = oEvent.getParameters();

                        // Handle Invalid Session error
                        if (oParams.response.responseText.indexOf("ZIH/146") >= 0) {
                            fnShowSessionExpiredError(oParams, oErrorTexts, oDisplayState, oRouter);
                        } else {
